// This code is reference to this github repo :https://github.com/svitkin/leaflet-timeline-slider
...
function markerSize(population) {
    return Math.sqrt(population) * 300;
}

function getYearColor(year) {
    const colors = ["#00A5E3", "#FFA23A", "#FF96C5", "#FF5768", "#FFBF65", "#6C88C4", "#FF3357", "#3357FF"];
    
    // Map the year to an index in the colors array
    const index = (year - 2011) % colors.length;
        ...
        for (let i= 0; i<data.length; i++) {
            zipCodeMarker.push(
                L.circle([data[i]['lat'], data[i]['lon']], {
                    stroke: false,
                    fillOpacity: 0.5,
                    color: getYearColor(label),
                    fillColor: getYearColor(label),
...
d3.json(years).then(data => { 
    years = data.map(x=>String(x.years))
